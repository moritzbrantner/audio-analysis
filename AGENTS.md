...
- Use `scripts/check-fast.sh` for the ordinary local library loop. Run `bash scripts/check-adapters.sh` when adapter shells change or when distribution compatibility is being checked. Repository CI retains full workspace/all-feature adapter coverage even though the local fast loop does not.
