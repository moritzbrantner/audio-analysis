...
- Use `scripts/check-fast.sh` for the ordinary library loop. Run `scripts/check-adapters.sh` when adapter shells change or when distribution compatibility is being checked.
