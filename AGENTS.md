...
- Before starting implementation, run `bash scripts/check-agent-readiness.sh`. It checks locked Cargo metadata and free build-disk capacity before model time is spent. Use `--with-source` when the task requires the exact Foundation source graph.
