//! Shared FFT, STFT, and spectral analysis owned by `audio-analysis-core`.
//!
//! Compatibility packages may re-export these symbols, but shared audio-domain
//! consumers should depend on this module directly.

use crate::{mono_samples, FrameSpec, WindowFunction};
...
impl SpectralScratch {
    ...
    fn load_windowed(&mut self, samples: &[f32], window: WindowFunction) {
        let len = self.buffer.len();
        for (index, value) in self.buffer.iter_mut().enumerate() {
            let sample = samples.get(index).copied().unwrap_or(0.0);
            *value = Complex::new(sample * window.coefficient(index, len), 0.0);
        }
    }
...
impl FourierTransform {
    ...
    pub fn analyze_samples(&self, samples: &[f32], sample_rate: u32) -> Result<Spectrum> {
        let mut scratch = SpectralScratch::new(self.fft_size);
        self.analyze_samples_with_scratch(samples, sample_rate, &mut scratch)
    }

    fn analyze_samples_with_scratch(
        &self,
        samples: &[f32],
        sample_rate: u32,
        ...
        scratch.load_windowed(samples, self.window);
        scratch.process();
...
pub fn spectrogram(
    ...
    let mut scratch = SpectralScratch::new(config.fft_size);
    let mut frames = Vec::new();

    for (start_sample, frame) in spec.frames(samples) {
        frames.push(SpectrogramFrame {
            start_sample,
            start_seconds: start_sample as f64 / sample_rate as f64,
            spectrum: transform.analyze_samples_with_scratch(
                frame,
                sample_rate,
                &mut scratch,
            )?,
...
///
/// This keeps the reusable FFT/phase primitive in the shared spectral core. It predicts
...
pub fn complex_spectral_difference(
    ...
    let mut scratch = SpectralScratch::new(fft_size);
    let mut start = 0_usize;
    let mut frame_index = 0_usize;

    while start < samples.len() {
        scratch.load_windowed(&samples[start..], WindowFunction::Hann);
        scratch.process();

        let mut frame_difference = 0.0_f32;
        for (bin_index, value) in scratch.buffer.iter().take(half_bins).enumerate().skip(1) {
            ...
            if frame_index > 0 {
                ...
                let predicted = Complex::from_polar(
