...
impl TempoRange {
    /// Creates a tempo range. Callers may construct this as a constant; public
    /// operations reject invalid ranges rather than panicking.
    ...
    pub fn playback_rate_bounds(self) -> Option<(f64, f64)> {
        self.is_valid().then(|| {
            (
                1.0 + self.min_percent / 100.0,
                1.0 + self.max_percent / 100.0,
            )
        })
    }
}
...
///
/// Finite out-of-range positions are clamped. Non-finite positions return
/// non-finite gains so callers can fail closed according to product policy.
...
///
/// The returned vector contains at most `point_count` entries and never more
/// entries than input samples. Non-finite samples are ignored and finite values
/// are clamped to the normalized `[-1, 1]` range.
...
mod tests {
    ...
    fn waveform_extrema_preserves_bucket_extremes_and_bounds() {
        ...
        assert_eq!(extrema[0], WaveformExtrema { min: 0.0, max: 0.75 });
        assert_eq!(extrema[1], WaveformExtrema { min: -1.0, max: 0.0 });
        assert_eq!(extrema[2], WaveformExtrema { min: 0.0, max: 1.0 });
    }
}
